      ...
      <div className="flex flex-wrap items-center gap-2 bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-2xl w-fit border border-slate-100 dark:border-slate-800">
        {(['all', 'quiz', 'sims', 'course'] as const).map(f => (
          <button 
            ...
            className={`px-8 py-2.5 rounded-xl m3-label-medium font-black uppercase tracking-[0.2em] transition-all ${activeFilter === f ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            {f}
          </button>
        ))}
              ...
              >
                <div className="flex items-center space-x-6">
                  ...
                  <div className="space-y-1.5 overflow-hidden">
                    <h4 className="m3-headline-small font-black text-slate-900 dark:text-white group-hover:text-primary transition-colors tracking-tighter uppercase truncate">{draft.title}</h4>
